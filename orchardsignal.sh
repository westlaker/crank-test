...
if [ "$USE_WRK" = "1" ]; then
	...
	# Find the PID of script2.sh
	pid=$(pgrep -f runtopdownsignal.sh)

	if [ -n "$pid" ]; then
	    echo "Sending signal (SIGUSR1) to runtopdownsignal.sh..."
	    kill -SIGUSR1 "$pid"
	else
	    echo "runtopdownsignal.sh is not running. No signal sent."
	fi
