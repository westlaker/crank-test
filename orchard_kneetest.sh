...
: ${APP_CPUSET:="26-57"}    # Affinity mask for OrcharCMS, e.g 0-14 (15 cores)
: ${WRK_CPUSET:="60"}      # Affinity mask for load generator. typically, just one core is enough, consider using more
...
if [ "$USE_WRK" = "1" ]; then
	...
	do
    taskset -c $WRK_CPUSET wrk -d 1s -c $WRK_CONNECTIONS http://$SERVER_ADDRESS:5014/about --latency --header "Accept: text/plain,text/html;q=0.9,application/xhtml+xml;q=0.9,application/xml;q=0.8,*/*;q=0.7" --header "Connection: keep-alive" 1>>gnp_stdout 2>>gnp_stderr
done
...
#  taskset -c $WRK_CPUSET wrk -d $RUN -c $WRK_CONNECTIONS http://$SERVER_ADDRESS:5014/about --latency --header "Accept: text/plain,text/html;q=0.9,application/xhtml+xml;q=0.9,application/xml;q=0.8,*/*;q=0.7" --header "Connection: keep-alive" > results${DOTNET_VER}_3.txt 3>&1
...
echo "RPS1: $(cat results${DOTNET_VER}_1.txt)"
